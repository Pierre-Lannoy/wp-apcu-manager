...
class Libraries {
	...
	public static function init() {
		...
		self::$psr4_libraries['psr-03']      = [
			'name'    => 'PSR-3',
			'prefix'  => 'Psr\\Log',
			'base'    => APCM_VENDOR_DIR . 'psr/log-v' . $psrlog_version . '/',
		...
		self::$psr4_libraries['psr-07']      = [
			...
			'base'    => APCM_VENDOR_DIR . 'psr/http-message/',
			'version' => '2.0',
			'author'  => 'PHP Framework Interop Group',
			'url'     => 'https://www.php-fig.org/',
			'license' => 'mit',
		...
		self::$psr4_libraries['psr-18']      = [
			...
			'base'    => APCM_VENDOR_DIR . 'psr/http-client/',
			'version' => '1.0.3',
			'author'  => 'PHP Framework Interop Group',
			'url'     => 'https://www.php-fig.org/',
			'license' => 'mit',
		...
		self::$psr4_libraries['decalog-sdk'] = [
			'name'    => 'DecaLog SDK',
			'prefix'  => 'DecaLog',
			'base'    => APCM_VENDOR_DIR . 'decalog-sdk/',
			'version' => '4.2.0',
	...
	private function license_name( $license ) {
		switch ( $license ) {
			case 'mit':
				$result = esc_html__( 'MIT license', 'apcu-manager' );
				break;
			case 'apl2':
				$result = esc_html__( 'Apache license, version 2.0', 'apcu-manager' );
				break;
			case 'gpl2':
				$result = esc_html__( 'GPL-2.0 license', 'apcu-manager' );
				break;
			case 'gpl3':
				$result = esc_html__( 'GPL-3.0 license', 'apcu-manager' );
				break;
			default:
				$result = esc_html__( 'unknown license', 'apcu-manager' );
				break;
		}
		return $result;
	}
	...
	public function sc_get_list( $attributes ) {
		...
		if ( 'html' === $style ) {
			...
			foreach ( $list as $library ) {
				...
				$items[] = sprintf( __( '<a href="%1$s">%2$s %3$s</a>%4$s by %5$s (%6$s)', 'apcu-manager' ), $library['url'], $library['name'], $library['version'], $library['langs'], $library['author'], $library['license'] );
			}
