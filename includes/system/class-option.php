...
class Option {
	...
	public static function init() {
		...
		self::$defaults['compat']            = true;
		self::$defaults['maxsize']           = 100;
		self::$network                       = [ 'forceearlyloading', 'earlyloading', 'compat', 'version', 'use_cdn', 'use_apcu',  'script_in_footer', 'display_nag', 'analytics', 'history', 'gc', 'metrics', 'adminbar' ];
	}
	/**
	...
	public static function site_exists( $option ) {
		return 'non_existent_option' !== get_option( APCM_PRODUCT_ABBREVIATION . '_' . $option, 'non_existent_option' );
	}
	...
	public static function network_exists( $option ) {
		return 'non_existent_option' !== get_site_option( APCM_PRODUCT_ABBREVIATION . '_' . $option, 'non_existent_option' );
	}
	...
	public static function site_set( $option, $value, $autoload = null ) {
		...
		return update_option( APCM_PRODUCT_ABBREVIATION . '_' . $option, $value, $autoload );
	}
	...
	public static function site_delete_all() {
		...
		$result = 0;
		// phpcs:ignore
		$delete = $wpdb->get_col( "SELECT option_name FROM {$wpdb->options} WHERE option_name LIKE '" . APCM_PRODUCT_ABBREVIATION . '_%' . "';" );
