...
class Assets {
	...
	public function prefetch() {
		if ( Option::network_get( 'use_cdn' ) && APCM_CDN_AVAILABLE ) {
	...
	public function register_style( $handle, $src, $file, $deps = [], $media = 'all' ) {
		if ( Option::network_get( 'use_cdn' ) && APCM_CDN_AVAILABLE ) {
			if ( APCM_ADMIN_URL === $src ) {
				$file = 'https://cdn.jsdelivr.net/wp/' . APCM_SLUG . '/tags/' . APCM_VERSION . '/admin/' . $file;
			} else {
				$file = 'https://cdn.jsdelivr.net/wp/' . APCM_SLUG . '/tags/' . APCM_VERSION . '/public/' . $file;
			}
			// phpcs:ignore
