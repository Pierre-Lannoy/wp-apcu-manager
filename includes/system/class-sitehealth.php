...
class Sitehealth {
	...
	public static function perfopsone_info( $debug_info ) {
		...
		if ( ! array_key_exists( $key, $debug_info ) ) {
			$debug_info[ $key ] = [
				'label'  => 'PerfOps One - ' . esc_html__( 'Object cache', 'apcu-manager' ),
		...
		if ( ! array_key_exists( $key, $debug_info ) ) {
			$debug_info[ $key ] = [
				'label'       => 'OPcache',
				'description' => esc_html__( 'OPcache settings and status', 'apcu-manager' ),
	...
	public static function plugin_info( $debug_info ) {
		$debug_info[ self::$slug ] = [
			'label'       => APCM_PRODUCT_NAME,
			'description' => esc_html__( 'Plugin diagnostic information', 'apcu-manager' ),
	...
	public static function perfopsone_test_objectcache( $tests ) {
		...
		if ( ! array_key_exists( $key, $tests['direct'] ) ) {
			$tests['direct'][ $key ] = [
				'label' => __( 'Object Cache Test', 'apcu-manager' ),
	...
	public static function perfopsone_test_objectcache_do() {
		...
		if ( 'apcu' === $analytics['type'] ) {
			return [
				'label'       => esc_html__( 'You should improve object caching', 'apcu-manager' ),
				'status'      => 'recommended',
				'badge'       => [
					'label' => esc_html__( 'Performance', 'apcu-manager' ),
					'color' => 'blue',
				],
				'description' => sprintf( '<p>%s %s</p>', esc_html__( 'APCu is available on your site, but only PerfOps One suite and some few other plugins know how to take advantage of it.', 'apcu-manager' ), sprintf( esc_html__( 'You should consider using %s to improve your site\'s speed.', 'apcu-manager' ), '<a href="https://perfops.one/apcu-manager/">APCu Manager</a>' ) ),
				'actions'     => '',
				'test'        => $key,
			];
		}
		if ( 'object_cache' === $analytics['type'] ) {
			return [
				'label'       => esc_html__( 'Your site uses object caching', 'apcu-manager' ),
				'status'      => 'good',
				'badge'       => [
					'label' => esc_html__( 'Performance', 'apcu-manager' ),
					'color' => 'blue',
				],
				'description' => sprintf( '<p>%s</p>', esc_html__( 'Your site uses a dedicated object caching mechanism. That\'s great.', 'apcu-manager' ) ),
				'actions'     => '',
				'test'        => $key,
			];
		}
		return [
			'label'       => esc_html__( 'You should use object caching', 'apcu-manager' ),
			'status'      => 'recommended',
			'badge'       => [
				'label' => esc_html__( 'Performance', 'apcu-manager' ),
				'color' => 'orange',
			],
			'description' => sprintf( '<p>%s %s</p>', esc_html__( 'Your site uses database transient.', 'apcu-manager' ), esc_html__( 'You should consider using a dedicated object caching mechanism, like APCu, Memcached or Redis, to improve your site\'s speed.', 'apcu-manager' ) ),
			'actions'     => '',
			'test'        => $key,
		];
	}
	...
	public static function perfopsone_test_opcache( $tests ) {
		...
		if ( ! array_key_exists( $key, $tests['direct'] ) ) {
			$tests['direct'][ $key ] = [
				'label' => __( 'OPcache Test', 'apcu-manager' ),
	...
	public static function perfopsone_test_opcache_do() {
		...
		if ( function_exists( 'opcache_invalidate' ) && function_exists( 'opcache_compile_file' ) && function_exists( 'opcache_is_script_cached' ) ) {
			$result = [
				'label'       => esc_html__( 'Your site uses OPcache', 'apcu-manager' ),
				'status'      => 'good',
				'badge'       => [
					'label' => esc_html__( 'Performance', 'apcu-manager' ),
					'color' => 'blue',
				],
				'description' => sprintf( '<p>%s</p>', esc_html__( 'Your site uses OPcache to improve PHP performance. That\'s great.', 'apcu-manager' ) ),
				'actions'     => '',
				'test'        => $key,
			];
		} else {
			$result = [
				'label'       => esc_html__( 'You should use OPcache', 'apcu-manager' ),
				'status'      => 'recommended',
				'badge'       => [
					'label' => esc_html__( 'Performance', 'apcu-manager' ),
					'color' => 'orange',
				],
				'description' => sprintf( '<p>%s</p>', esc_html__( 'You should consider using OPcache. It would improve PHP performance of your site.', 'apcu-manager' ) ),
				'actions'     => '',
				'test'        => $key,
			];
		}
	...
	public static function perfopsone_test_shmop( $tests ) {
		...
		if ( ! array_key_exists( $key, $tests['direct'] ) ) {
			$tests['direct'][ $key ] = [
				'label' => __( 'Shared Memory Test', 'apcu-manager' ),
	...
	public static function perfopsone_test_shmop_do() {
		...
		if ( function_exists( 'shmop_open' ) && function_exists( 'shmop_read' ) && function_exists( 'shmop_write' ) && function_exists( 'shmop_delete' ) && function_exists( 'shmop_close' ) ) {
			$result = [
				'label'       => esc_html__( 'Your site can use shared memory', 'apcu-manager' ),
				'status'      => 'good',
				'badge'       => [
					'label' => esc_html__( 'Performance', 'apcu-manager' ),
					'color' => 'blue',
				],
				'description' => sprintf( '<p>%s</p>', esc_html__( 'Your site can use shared memory to allow inter-process communication. That\'s great.', 'apcu-manager' ) ),
				'actions'     => '',
				'test'        => $key,
			];
		} else {
			$result = [
				'label'       => esc_html__( 'You should allow inter-process communication', 'apcu-manager' ),
				'status'      => 'recommended',
				'badge'       => [
					'label' => esc_html__( 'Performance', 'apcu-manager' ),
					'color' => 'orange',
				],
				'description' => sprintf( '<p>%s</p>', esc_html__( 'You should consider using shared memory (PHP shmop) to allow inter-process communication.', 'apcu-manager' ) ),
				'actions'     => '',
				'test'        => $key,
			];
		}
	...
	public static function perfopsone_test_i18n( $tests ) {
		...
		if ( ! array_key_exists( $key, $tests['direct'] ) ) {
			$tests['direct'][ $key ] = [
				'label' => __( 'I18n Extension Test', 'apcu-manager' ),
	...
	public static function perfopsone_test_i18n_do() {
		...
		if ( I18n::is_extension_loaded() ) {
			$result = [
				'label'       => esc_html__( 'Your site uses Intl extension', 'apcu-manager' ),
				'status'      => 'good',
				'badge'       => [
					'label' => esc_html__( 'Internationalization', 'apcu-manager' ),
					'color' => 'blue',
				],
				'description' => sprintf( '<p>%s</p>', esc_html__( 'Your site uses PHP Intl extension to improve localization features. That\'s great.', 'apcu-manager' ) ),
				'actions'     => '',
				'test'        => $key,
			];
		} else {
			$result = [
				'label'       => esc_html__( 'You should use Intl extension', 'apcu-manager' ),
				'status'      => 'recommended',
				'badge'       => [
					'label' => esc_html__( 'Internationalization', 'apcu-manager' ),
					'color' => 'blue',
				],
				'description' => sprintf( '<p>%s</p>', esc_html__( 'You should consider using PHP Intl extension. It would improve localization features on your site.', 'apcu-manager' ) ),
				'actions'     => '',
				'test'        => $key,
			];
		}
