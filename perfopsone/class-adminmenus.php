		...
		public static function initialize() {
			foreach ( apply_filters( 'init_perfops_admin_menus', [] ) as $menu => $submenus ) {
				if ( ! in_array( 'perfopsone-' . $menu, self::$slugs, true ) ) {
					switch ( $menu ) {
						case 'analytics':
							add_menu_page( esc_html__( 'Dashboard', 'apcu-manager' ), sprintf( esc_html__( '%s Analytics', 'apcu-manager' ), 'PerfOps' ), 'manage_options', 'perfopsone-' . $menu, [ self::class, 'get_analytics_page' ], 'dashicons-chart-bar', 6666 );
							...
							break;
						case 'tools':
							add_menu_page( esc_html__( 'Available Tools', 'apcu-manager' ), sprintf( esc_html__( '%s Tools', 'apcu-manager' ), 'PerfOps' ), 'manage_options', 'perfopsone-' . $menu, [ self::class, 'get_tools_page' ], 'dashicons-admin-tools', 6666 );
							...
							break;
						case 'insights':
							add_menu_page( esc_html__( 'Available Reports', 'apcu-manager' ), sprintf( esc_html__( '%s Insights', 'apcu-manager' ), 'PerfOps' ), 'manage_options', 'perfopsone-' . $menu, [ self::class, 'get_insights_page' ], 'dashicons-lightbulb', 6666 );
							...
							break;
						case 'records':
							add_menu_page( esc_html__( 'Available Catalogues', 'apcu-manager' ), sprintf( esc_html__( '%s Records', 'apcu-manager' ), 'PerfOps' ), 'manage_options', 'perfopsone-' . $menu, [ self::class, 'get_records_page' ], 'dashicons-book', 6666 );
							...
							break;
						case 'settings':
							add_menu_page( esc_html__( 'Control Center', 'apcu-manager' ), sprintf( esc_html__( '%s Settings', 'apcu-manager' ), 'PerfOps' ), 'manage_options', 'perfopsone-' . $menu, [ self::class, 'get_settings_page' ], 'dashicons-admin-settings', 6666 );
							...
							break;
						case 'consoles':
							add_menu_page( esc_html__( 'Available Consoles', 'apcu-manager' ), sprintf( esc_html__( '%s Consoles', 'apcu-manager' ), 'PerfOps' ), 'manage_options', 'perfopsone-' . $menu, [ self::class, 'get_consoles_page' ], 'dashicons-embed-generic', 6666 );
							add_submenu_page( 'perfopsone-' . $menu, esc_html__( 'Available Consoles', 'apcu-manager' ), __( 'Available Consoles', 'apcu-manager' ), 'manage_options', 'perfopsone-' . $menu, [ self::class, 'get_sconsoles_page' ], 0 );
							break;
					}
			...
			if ( array_key_exists( 'analytics', self::$menus ) ) {
				...
				foreach ( self::$menus['analytics'] as $item ) {
					...
					if ( $item['activated'] ) {
						...
						$i['text'] = esc_html__( 'This analytics feature is currently disabled. Click here to activate it.', 'apcu-manager' );
						$i['url']  = $item['remedy'];
					}
					$items[] = $i;
				}
		...
		public static function get_settings_page() {
			if ( array_key_exists( 'settings', self::$menus ) ) {
				...
				foreach ( self::$menus['settings'] as $item ) {
					...
					if ( $d->waiting_update() ) {
						$i['need_update'] = sprintf( esc_html__( 'Need to be updated to %s.', 'apcu-manager' ), $d->waiting_update() );
					} else {
						$i['need_update'] = '';
					}
					if ( $d->is_required_wp_ok() ) {
						$i['need_wp_update'] = '';
						$i['ok_wp_update']   = esc_html__( 'OK', 'apcu-manager' );
					} else {
						$i['need_wp_update'] = esc_html__( 'need update', 'apcu-manager' );
						$i['ok_wp_update']   = '';
					}
					if ( $d->is_required_php_ok() ) {
						$i['need_php_update'] = '';
						$i['ok_php_update']   = esc_html__( 'OK', 'apcu-manager' );
					} else {
						$i['need_php_update'] = esc_html__( 'need update', 'apcu-manager' );
						$i['ok_php_update']   = '';
					}
					$i['icon'] = call_user_func( $item['icon_callback'] );
					$i['slug'] = $item['plugin'];
					...
					if ( 0 < (int) $i['installs'] ) {
						$i['installs'] = sprintf( esc_html__( '%s+ installs.', 'apcu-manager' ), Conversion::number_shorten( (int) $i['installs'], 0 ) );
					} else {
						$i['installs'] = '';
					}
					if ( 0 < (int) $i['downloads'] ) {
						$i['downloads'] = sprintf( esc_html__( '%s downloads.', 'apcu-manager' ), Conversion::number_shorten( (int) $i['downloads'], 2 ) );
					} else {
						$i['downloads'] = '';
					}
					if ( 0 < (int) $i['reviews'] ) {
						$i['reviews'] = sprintf( esc_html__( '%s reviews.', 'apcu-manager' ), Conversion::number_shorten( (int) $i['reviews'], 0 ) );
						$i['rating']  = $i['rating'] / 20;
					} else {
						$i['reviews'] = esc_html__( 'No review yet.', 'apcu-manager' );
						$i['rating']  = 0.0;
					}
					$i['stars'] = '';
					if ( 0 < (int) $i['rating'] ) {
